...
/obj/item/weapon/gun/medbeamb
	name = "Prototype Medical Beamgun"
	desc = "Crossing the streams is ill-advised."
	icon = 'icons/obj/chronos.dmi'
	icon_state = "chronogunb"
	item_state = "chronogunb"
	w_class = WEIGHT_CLASS_NORMAL
...
/obj/item/weapon/gun/medbeamb/New()
	..()
	START_PROCESSING(SSobj, src)

/obj/item/weapon/gun/medbeamb/dropped(mob/user)
	..()
	LoseTarget()

/obj/item/weapon/gun/medbeamb/equipped(mob/user)
	..()
	LoseTarget()

/obj/item/weapon/gun/medbeamb/proc/LoseTarget()
	if(active)
		qdel(current_beam)
		active = 0
		on_beam_release(current_target)
...
/obj/item/weapon/gun/medbeamb/process_fire(atom/target as mob|obj|turf, mob/living/user as mob|obj, message = 1, params, zone_override)
	if(isliving(user))
		add_fingerprint(user)
	...
	current_target = target
	active = 1
	current_beam = new(user,current_target,time=6000,beam_icon_state="medbeamb",btype=/obj/effect/ebeam/medical)
	addtimer(CALLBACK(current_beam, /datum/beam.proc/Start), 0)

	feedback_add_details("gun_fired","[src.type]")

/obj/item/weapon/gun/medbeamb/process()
...
/obj/item/weapon/gun/medbeamb/proc/los_check(atom/movable/user, mob/target)
	var/turf/user_turf = user.loc
	if(mounted)
		user_turf = get_turf(user)
	else if(!istype(user_turf))
...
/obj/item/weapon/gun/medbeamb/proc/on_beam_hit(var/mob/living/target)
	return

/obj/item/weapon/gun/medbeamb/proc/on_beam_tick(var/mob/living/target)
	if(target.health != target.maxHealth)
		PoolOrNew(/obj/effect/overlay/temp/heal, list(get_turf(target), "#80F5FF"))
...
/obj/item/weapon/gun/medbeamb/proc/on_beam_release(var/mob/living/target)
	return

/obj/effect/ebeam/medicalb
	name = "medical beam"
