var/const/WASTELAND			=(1<<3)

var/const/GENERAL			=(1<<0)
var/const/NCR_SERGEANT		=(1<<1)
var/const/RANGER			=(1<<2)
var/const/TROOPER			=(1<<3)
//var/const/CCO				=(1<<4)

var/const/RECRUIT			=(1<<4)
var/const/PRIME				=(1<<5)
var/const/LEGDECAN			=(1<<6)
var/const/LEGVEX			=(1<<7)
var/const/LEGCENTURION		=(1<<8)
var/const/LEGLEGAT			=(1<<9)

var/const/SHERIFF			=(1<<10)
var/const/SETTLER			=(1<<11)
var/const/ADVENTURER		=(1<<12)
var/const/RAIDER        	=(1<<13)
var/const/MAYOR				=(1<<14)

//This jobs using medsci flag, cause me too lazy.

var/const/COLONEL				=(1<<1)
var/const/ENCLAVE_LIEUTENANT	=(1<<2)
var/const/ENCLAVE_PRIVATE		=(1<<3)

var/const/MERCENARY			=(1<<5)
var/const/GANGLEAD			=(1<<6)
var/const/GHOUL_MASTER		=(1<<7)
var/const/GHOUL_HAND		=(1<<8)
var/const/GHOUL_PRIEST		=(1<<9)


//engsec

var/const/ELDER				=(1<<1)
var/const/PALADIN			=(1<<2)
var/const/KNIGHT			=(1<<3)
var/const/SCRIBE            =(1<<4)

var/const/CITIZEN			=(1<<5)

var/const/OVERSEER			=(1<<6)
var/const/VAULTENGINEER		=(1<<7)
var/const/VAULTOFFICER		=(1<<8)
var/const/VAULTDOCTOR		=(1<<9)
var/const/VAULTDWELLER		=(1<<10)