...
var/global/datum/error_viewer/error_cache/error_cache = new /datum/error_viewer/error_cache()
...
/datum/error_viewer/error_cache/show_to(user, datum/error_viewer/back_to, linear)
	...
	html += "<b>[total_runtimes]</b> runtimes, <b>[total_runtimes_skipped]</b> skipped<br><br>"
	if (!linear)
...
/datum/error_viewer/error_cache/proc/log_error(exception/e, list/desclines, skip_count)
	...
	if (error_source.next_message_at <= world.time)
		...
		var/err_msg_delay
		if(config)
			// Hardcoded, not importing the config
