...
/mob/living/silicon/robot/sesurity
	...
	var/playstyle_string = "<span class='userdanger'>You are a Syndicate assault cyborg!</span><br>\
							...
							Your cyborg LMG will slowly produce ammunition from your power supply, and your operative pinpointer will find and locate fellow nuclear operatives. \
							<i>Help the operatives secure the disk at all costs!</i></b>"
	var/set_module = /obj/item/weapon/robot_module/sesurity

/mob/living/silicon/robot/sesurity/New(loc)
	..()
	cell.maxcharge = INFINITY
	cell.charge = INFINITY
	radio = new /obj/item/device/radio/borg/syndicate(src)
...
/mob/living/silicon/robot/sesurity/bullet_act(obj/item/projectile/P)
	if(istype(P, /obj/item/projectile/bullet))
		var/reflectchance = 85 - round(P.damage/3)
		if(prob(reflectchance))
