...
/obj/machinery/trading_machine
	...
	var/create_inventory = 1
	var/basic_price = 20
	var/expected_price = 0
	var/obj/item/vending_item
	var/item_not_acceptable_message = "Something wrong... Can't insert item."
...
/obj/machinery/trading_machine/proc/get_paper_description_data()
	...
	data += "Wasteland Trading Company поздравляет вас с приобретением нашего торгового автомата! Вы в одном шаге от богатсва! <br><br>Ознакомтесь с нашей инструкцией и вперед - зарабатывать крышки!<br><br>"
	data += "1. Найдите ключ, подписаный именем автомата. Он должен лежать не далеко от самого автомата.<br>"
	data += "2. Откройте автомат ключом, чтобы перевести его в сервисный режим. <br>"
	data += "3. Не забудьте закрыть панель автомата ключем, чтобы перевести его обратно в режим торговли.<br><br>"

	data += "Чтобы выставить предмет на продажу:<br>"
	...
	data += "4. Закройте ключем сервисную панель.<br><br>"

	data += "Чтобы достать заработаные крышки:<br>"
	data += "1. Откройте сервисную панель автомата ключем.<br>"
	data += "2. В меню нажмите на кнопку Unload напротив количества крышек.<br>"
	data += "Крышки окажутся на земле.<br><br>"

	data += "Чтобы изменить доступ к автомату:<br>"
	data += "1. Откройте сервисную панель автомата ключем.<br>"
	data += "2. Открутите отверткой крепление замка. <br>"
	...
	data +=	"7. Вставьте замок обратно в автомат. <br>"
