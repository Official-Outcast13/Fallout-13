		...
		for(z in z1 to z2)
			...
			for(y in y1 to y2)
				...
				for(x in x1 to x2)
					...
					if(T.loc!=defarea) to_chat(S["AREA"], areas[T.loc])
					T.Write(S)
					S.cd=".."
				S.cd=".."
			sleep()
