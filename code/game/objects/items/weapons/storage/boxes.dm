...
/obj/item/weapon/match/proc/strikelight(obj/item/W as obj, mob/user as mob)//Making this a seperate proc so it can be called on any object.
	if(src.lit)
		return
	if(prob(50))//50% chance to light.
		user.visible_message("<span class='notice'>[user] strikes \the [W] with the match and it lights!</span>")
		playsound(W, 'sound/f13items/matchstick_lit.ogg', 40, 1, -1)
		src.matchignite()
	else
		user.visible_message("<span class='notice'>[user] strikes \the [W] with the match.</span>")
		playsound(W, 'sound/f13items/matchstick_hit.ogg', 40, 1, -1)
...
/obj/item/weapon/storage/box/lethalshot
	...
	desc = "A box standard 12 gauge buckshot."
